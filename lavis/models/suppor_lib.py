...
def plot_scanpaths(scanpaths, img_path, lengths, save_path, img_height=128, img_witdth=256):
    ...
    for user_i in range(lengths.size):#(20):
        idx1 = int(user_i / 5)
        idx2 = user_i % 5
        ...
        for sample_i in range(lengths[user_i]):
            points_x.append(scanpaths[user_i][sample_i][0])
            points_y.append(scanpaths[user_i][sample_i][1])
...
def plot_the_first_scanpath(scanpaths, img_path, lengths, save_path, img_height=128, img_witdth=256):
    # Plot predicted scanpaths
    # this code is on the basis of ScanGAN https://github.com/DaniMS-ZGZ/ScanGAN360/
    ...
    for sample_i in range(lengths[0]):
        points_x.append(scanpaths[0][sample_i][0])
        points_y.append(scanpaths[0][sample_i][1])

    previous_point = None

    # plot 20 scanpaths for an image.
    for user_i in range(20):#(20):

        colors = cm.rainbow(np.linspace(0, 1, len(points_x)))

        try:
            tempx = points_x[user_i]
        except:
            break

        tempx = points_x[user_i]
        tempy = points_y[user_i]
        c = colors[user_i]
        x = tempx * img_witdth
        y = tempy * img_height
        markersize = 28.
        if previous_point is not None:
            if abs(previous_point[0] - x) < (img_witdth / 2):
                plt.plot([x, previous_point[0]], [y, previous_point[1]], color='blue', linewidth=8.,
                                     alpha=0.35)
            else:
                h_diff = (y - previous_point[1]) / 2
                if (x > previous_point[0]):  # X is on the right, Previous is on the Left
                    ...
                    plt.plot([previous_point[0], img_witdth],
                                         [previous_point[1], previous_point[1] + h_diff], color='blue',
                                         linewidth=8., alpha=0.35)
                    plt.plot([0, x], [previous_point[1] + h_diff, y], color='blue', linewidth=8.,
                                         alpha=0.35)
        previous_point = [x, y]
        plt.plot(x, y, marker='o', markersize=markersize, color=c, alpha=.8)
        plt.axis('off')
        plt.savefig(save_path + '/sp_' + str(user_i) + image_name + ".png")
